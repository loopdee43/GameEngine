...
GLuint LoadShaders(const char * vertex_file_path, const char * fragment_file_path) {
    ...
    long VertexShaderStreamSize = ftell(VertexShaderStream);
    fseek(VertexShaderStream, 0, SEEK_SET);
    char *VertexShaderCode = (char *)malloc(VertexShaderStreamSize + 1);
    fread(VertexShaderCode, 1, VertexShaderStreamSize, VertexShaderStream);
    fclose(VertexShaderStream);
    VertexShaderCode[VertexShaderStreamSize] = '\0';
    ...
    long FragmentShaderStreamSize = ftell(FragmentShaderStream);
    fseek(FragmentShaderStream, 0, SEEK_SET);
    char *FragmentShaderCode = (char *)malloc(FragmentShaderStreamSize + 1);
    fread(FragmentShaderCode, 1, FragmentShaderStreamSize, FragmentShaderStream);
    fclose(FragmentShaderStream);
    FragmentShaderCode[FragmentShaderStreamSize] = '\0';
